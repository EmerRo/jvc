...
require_once "app/Services/VentaReporteService.php";
...
class ReportesVentaController extends Controller
{
  ...
  public function __construct()
  {
    ...
    $this->service = new VentaReporteService();
  }
  ...
  public function reporteVentaPorProducto()
  {
    ...
    $empresa = $this->service->obtenerEmpresa($_SESSION['id_empresa']);
    ...
    $rows = $this->service->obtenerReporteVentasPorProducto(
      $_GET['codprod'] ?? '',
      $_GET['fecha1'] ?? '',
      !empty($_GET['fecha2']) ? $_GET['fecha2'] : null
    );
  ...
  public function reporteCliente($id)
  {
    ...
    $cliente = $this->service->obtenerCliente($id);

    if (!$cliente) {
      throw new Exception("Cliente no encontrado");
    }

    $result = $this->service->obtenerVentasPorContacto($id, 0);

    $empresa = $this->service->obtenerEmpresa($cliente['id_empresa']);
  ...
  public function reporteProductos($id)
  {
    ...
    if (isset($rpart[1]) && $rpart[1] == 'nn') {
      $stmt = $this->conexion->prepare("SELECT pv.id_producto,c.datos,c.documento,v.id_venta,v.serie,v.numero,v.fecha_emision,pv.cantidad,pv.precio FROM ventas v 
        JOIN productos_ventas pv ON v.id_venta = pv.id_venta
        LEFT JOIN clientes c ON c.id_cliente= v.id_cliente 
        WHERE pv.id_producto = ? AND concat(year(v.fecha_emision),month(v.fecha_emision)) = ?");
      $stmt->bind_param("is", $id, $rpart[0]);
    } else {
      $periodo = $rpart[0] . ($rpart[1] ?? '');
      $stmt = $this->conexion->prepare("SELECT pv.id_producto,c.datos,c.documento,v.id_venta,v.serie,v.numero,v.fecha_emision,pv.cantidad,pv.precio FROM ventas v 
        JOIN productos_ventas pv ON v.id_venta = pv.id_venta
        LEFT JOIN clientes c ON c.id_cliente= v.id_cliente 
        WHERE pv.id_producto = ? AND concat(year(v.fecha_emision),month(v.fecha_emision), day(v.fecha_emision)) = ?");
      $stmt->bind_param("is", $id, $periodo);
    }
    $stmt->execute();
  ...
  public function comprobanteNotaE($venta, $nombreXML = '')
  {
    ...
    $datoVenta = $this->service->obtenerNotaElectronica($venta);
    $datoEmpresa = $this->service->obtenerEmpresa($_SESSION['id_empresa']);
    ...
    $rowVS = $this->service->obtenerNotaElectronicaSunat($venta);
    $qrImage = '';
    $hash_Doc = '';
    if ($rowVS) {
...
$listaProd1 = $this->service->obtenerProductosVentaSimple($venta);
    ...
    if ($datoVenta["id_tipo_pago"] == '2') {
      $rowTempCuo = '';
      $resulTempCuo = $this->service->obtenerCuotasVenta($venta);
      $contadorCuota = 0;
      $menosRowsNumH = 1;
    ...
    $qrImage = '';
    $hash_Doc = '';
    if ($rowVS = $this->service->obtenerVentaSunat($venta)) {
      $hash_Doc = "HASH: " . $rowVS['hash'] . "<br>";
      $qrCode = new QrCode($rowVS["qr_data"]);
      $qrCode->setSize(150);
      $image = $qrCode->writeString(); //Salida en formato de texto
    ...
    $this->mpdf->WriteFixedPosHTML("<div ><img style='height: 95px;width: 360px;' src='" .
      ImageStorage::url('empresas', $datoEmpresa['logo']) . "'></div>", 15, 5, 100, 120);
    ...
    try {
      $equiposVentaCheck = $this->service->verificarVentaTieneEquipos($venta);
  ...
  private function comprobanteVentaGen($dist, $venta, $nombreXML)
  {
    ...
    if ($guiaId) {
      $listaProd1 = $this->service->obtenerProductosVenta($venta, $guiaId);
    } else {
    ...
    if ($datoVenta['pagado2']) {
      $isSEgundoPago = true;
      $metodo2 = $this->service->obtenerMetodoPago($datoVenta['medoto_pago2_id']);
      $metodo1 = $this->service->obtenerMetodoPago($datoVenta['medoto_pago_id']);
...
<table style="width: 100%; border-collapse: collapse; border: 0.5px solid #373435; margin: 0; padding: 0;font-family: Calibri, Helvetica Neue, sans-serif; font-size: 9px;">
    ...
    $stmt = $this->conexion->prepare("SELECT u.* FROM usuarios u JOIN ventas v ON u.usuario_id = v.id_vendedor WHERE v.id_venta = ?");
...
<div style='width: 100%;'>
  ...
  public function imprimirvoucher5_6cm($id)
  {
    ...
    $dataVenta = $this->service->obtenerVenta($id);
    $igv_venta_sel = $dataVenta['igv'];
    $dataEmpresa = $this->service->obtenerEmpresa($dataVenta['id_empresa']);
    $dataCliente = $this->service->obtenerCliente($dataVenta['id_cliente']);
    $dataProVenta = $this->service->obtenerProductosVentaSimple($id);
    $dataServVenta = $this->service->obtenerServiciosVenta($id);

    $guiaRealionada = '';
    if ($rowGuia = $this->service->obtenerGuiaRemision($id)) {
      $guiaRealionada = $rowGuia["serie"] . '-' . Tools::numeroParaDocumento($rowGuia["numero"], 6);
    }
    ...
    if ($dataVenta["id_tipo_pago"] == '2') {
      $rowTempCuo = '';
      $resulTempCuo = $this->service->obtenerCuotasVenta($id);
      $contadorCuota = 0;
      $menosRowsNumH = 10;
      foreach ($resulTempCuo as $cuotTemp) {
        $menosRowsNumH += 11;
...
<div style="width: 100%;">
    ...
    $rowVS = $this->service->obtenerVentaSunat($id);
    $qrImage = '';
    if ($rowVS) {
      $qrCode = new QrCode($rowVS["qr_data"]);
      $qrCode->setSize(150);
    ...
    /*var_dump($totalIGVNumeros);
      die();*/
    $datoVenta = $this->service->obtenerVenta($id);
    $datoSucursal = $this->service->obtenerSucursal($datoVenta['sucursal'], $datoVenta['id_empresa']);
    if ($datoVenta['sucursal'] != '1') {
      if (!is_null($datoSucursal)) {
...
<table style='width:100%;margin-bottom: 10px'>
  <tr>
    <td align='center'>
      <img style=' max-width: 80%;' src='" . ImageStorage::url('empresas', $dataEmpresa['logo']) . "'>
</td>
</tr>
</table>
    <div style='width: 100%;text-align: center'>
...
";
  ...
  public function imprimirvoucher8cm($id)
  {
    ...
    $cajero = $this->service->obtenerUsuario($dataVenta['id_vendedor'] ?? 0) ?: [];
    $vendor = $this->service->obtenerUsuario(0) ?: [];

    $trCajero = "";
    $trVendor = "";
    if (!empty($cajero['nombres'])) {
      $trCajero = " <tr><td style='font-size: 11px'><strong>Cajero:</strong></td><td style='font-size: 11px'>{$cajero['nombres']}</td></tr>";
    }
    if (!empty($vendor['nombres'])) {
      $trVendor = " <tr><td style='font-size: 11px'><strong>Vendedor:</strong></td><td style='font-size: 11px'>{$vendor['nombres']}</td></tr>";
    }
...
<table style='width:100%;margin-bottom: 10px'>
  <tr>
    <td align='center'>
      <img style=' max-width: 85%;' src='" . ImageStorage::url('empresas', $dataEmpresa['logo']) . "'>
</td>
</tr>
</table>
    <div style='width: 100%;text-align: center'>
