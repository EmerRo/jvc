...
class OtroArchivoPDF extends Controller
{
  ...
  public function generarVistaPreviaPDF($titulo, $contenido, $header_image, $footer_image)
    ...
    public function generarOtroArchivoPDFBase64($id_archivo)
    {
        try {
            ...
            // Si no hay imágenes específicas, usar las de la plantilla
            if (!$headerImageUrl || !$footerImageUrl) {
                $template = new OtroArchivoTemplate();
                $template->obtenerTemplateActual();
            ...
            $html = $this->generarHTMLOtroArchivo($archivo, $headerImageUrl, $footerImageUrl, $numeroCorrelativo);
            ...
            error_log("Error generando PDF base64 de otro archivo: " . $e->getMessage());
            throw new Exception("Error al generar el PDF: " . $e->getMessage());
        }
    }

    // Método auxiliar para generar HTML de otro archivo
    private function generarHTMLOtroArchivo($archivo, $headerImageUrl, $footerImageUrl, $numeroCorrelativo)
    {
        $html = $this->generarEncabezado($headerImageUrl);
        
        // Agregar contenido específico de otro archivo
        $html .= "
        <div style='padding: 20px 15mm; min-height: calc(100vh - 200px);'>
            <table style='width: 100%; border-spacing: 0; font-family: Arial, sans-serif;'>
                <tr>
                    <td style='font-weight: bold; padding: 5px 0;'>Documento N°:</td>
