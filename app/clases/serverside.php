...
class TableData
{
	...
	public function getTallerCotizaciones($table, $index_column, $columns, $isExtra = false, $condicional = "", $where = false)
	{
		// Paging
		$sLimit = "";
		if (isset($_GET['iDisplayStart']) && $_GET['iDisplayLength'] != '-1') {
		...
		// Orden por defecto para cotizaciones del taller: por número descendente (más recientes primero)
		if (empty($sOrder)) {
		...
		// Individual column filtering parameters
		for ($i = 0; $i < count($columns); $i++) {
			if (isset($_GET['bSearchable_' . $i]) && $_GET['bSearchable_' . $i] == "true" && $_GET['sSearch_' . $i] != '') {
				$statement->bindValue(':search' . $i, '%' . $_GET['sSearch_' . $i] . '%', PDO::PARAM_STR);
			}
		...
		// Data set length after filtering
		$resultFilterTotal = $this->_db->query('SELECT FOUND_ROWS()');
		$iFilteredTotal = $resultFilterTotal->fetchColumn();

		// Total data set length
		$resultTotal = $this->_db->query('SELECT COUNT(`' . $index_column . '`) FROM `' . $table . '`');
		$iTotal = $resultTotal->fetchColumn();
		...
		foreach ($result as $aRow) {
			$row = array();
			for ($i = 0; $i < count($columns); $i++) {
				if (isset($aRow[$columns[$i]]) && $columns[$i] != ' ') {
					$row[] = ($aRow[$columns[$i]] == "0") ? '-' : $aRow[$columns[$i]];
				} else if ($columns[$i] != ' ') {
					$row[] = $aRow[$columns[$i]];
				}
