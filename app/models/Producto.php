...
class Producto
{
    ...
    public function verFilas($almacen)
{
    // Verificar si la variable de sesión sucursal existe, si no usar 1 como predeterminado
    $sucursal = isset($_SESSION['sucursal']) ? $_SESSION['sucursal'] : 1;
    
    $sql = "SELECT * from productos where id_empresa = '$this->id_empresa' and sucursal = '$sucursal' and estado = '1' AND almacen=$almacen order by id_producto DESC";
    ...
    if ($resultado && is_object($resultado) && method_exists($resultado, 'num_rows')) {
        error_log("Número de productos encontrados: " . $resultado->num_rows);
    } else {
        error_log("No se obtuvieron resultados o el resultado no es un objeto mysqli_result");
    }
