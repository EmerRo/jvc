...
<div class="mb-4">
    ...
    <button class="btn bg-rojo hover:bg-white" onclick="window.otroArchivoModuleInstance.reiniciar()">
...
<script>
    ...
    const otrosArchivosConfig = {
        tipo: 'otroArchivo',
        urls: {
            render: _URL + "/ajs/otroArchivo/render",
            insertar: _URL + "/ajs/otroArchivo/insertar",
            editar: _URL + "/ajs/otroArchivo/editar",
            borrar: _URL + "/ajs/otroArchivo/borrar",
            getOne: _URL + "/ajs/otroArchivo/getOne",
            generarPDF: _URL + "/ajs/otroArchivo/generarPDF",
            vistaPrevia: _URL + "/ajs/otroArchivo/vista-previa",
            obtenerTemplate: _URL + "/ajs/otroArchivo/obtener-template",
            guardarTemplate: _URL + "/ajs/otroArchivo/guardar-template",
            obtenerMembretes: _URL + "/ajs/otroArchivo/obtener-membretes",
            guardarMembretes: _URL + "/ajs/otroArchivo/guardar-membretes",
            obtenerTipos: _URL + "/ajs/otroArchivo/obtener-tipos-otroArchivos"
        },
        elementos: {
            // Botones principales
    ...
    function agregarTipoOtroArchivo() {
        ...
        $.ajax({
            url: _URL + "/ajs/otroArchivo/insertar-tipo-archivo",
            method: "POST",
            data: { nombre: nombre },
            dataType: 'json',
            success: function (data) {
    ...
    function guardarTipoOtroArchivoEditado() {
        ...
        $.ajax({
            url: _URL + "/ajs/otroArchivo/editar-tipo-archivo",
            method: "POST",
    ...
    function eliminarTipoOtroArchivo(id, nombre) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: _URL + "/ajs/otroArchivo/eliminar-tipo-archivo",
                    method: "POST",
                    data: { id: id },
                    dataType: 'json',
                    success: function (data) {
    ...
    function compartirWhatsAppOtroArchivo(id) {
        console.log('Compartiendo otro archivo por WhatsApp:', id);
        window.otroArchivoActualWhatsApp = id;
        $('#compartirWhatsAppOtroArchivoModal').modal('show');
    }

    function enviarWhatsAppOtroArchivo() {
        const numero = $('#numeroWhatsAppOtroArchivo').val().trim();
        const mensaje = $('#mensajeWhatsAppOtroArchivo').val().trim();
        ...
        $.ajax({
            url: _URL + '/ajs/otroArchivo/compartir-whatsapp',
            method: 'POST',
            data: {
                id_otroArchivo: window.otroArchivoActualWhatsApp,
                numero: numero,
                mensaje: mensaje
            },
            success: function(response) {
                if (response.res) {
                    $('#compartirWhatsAppOtroArchivoModal').modal('hide');
                    Swal.close();
...
<div class="modal fade" id="compartirWhatsAppOtroArchivoModal" tabindex="-1" aria-labelledby="compartirWhatsAppOtroArchivoModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="compartirWhatsAppOtroArchivoModalLabel">
                    <i class="fab fa-whatsapp text-success me-2"></i>Compartir Otro Archivo por WhatsApp
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="mb-3">
                    <label for="numeroWhatsAppOtroArchivo" class="form-label">
                        <i class="fas fa-phone me-1"></i>Número de WhatsApp
                    </label>
                    <input type="text" class="form-control" id="numeroWhatsAppOtroArchivo" 
                           placeholder="Ingrese el número sin +51 (ej: 999888777)" maxlength="9">
                </div>
                <div class="mb-3">
                    <label for="mensajeWhatsAppOtroArchivo" class="form-label">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                <button type="button" class="btn btn-success" onclick="enviarWhatsAppOtroArchivo()">
