<!-- resources\views\fragment-views\cliente\compra-edit.php -->
<?php
// ID de la compra a editar
    ...
    <div class="row align-items-center">
        <div class="col-md-8">
            ...
            <ol class="breadcrumb m-0">
                ...
                <li class="breadcrumb-item active" aria-current="page" style="color: #CA3438">Editar Orden Compra</li>
            </ol>
        </div>
        <div class="col-md-4">
    ...
    $(document).ready(function () {
        ...
        var app = new Vue({
            ...
            created() {
                ...
                const compraIdEdit = document.getElementById('compra_id_edit');
                if (compraIdEdit && compraIdEdit.value) {
                    this.cargarDatosCompra(compraIdEdit.value);
                }
            },
            ...
            methods: {
                    ...
                    $.ajax({
                        ...
                        success: function(resp) {
                            ...
                            if (data.res) {
                                ...
                                vue.venta.tipo_doc = compra.id_tido;
                                vue.venta.tipo_pago = compra.id_tipo_pago;
                                vue.venta.fecha = compra.fecha_emision;
                                vue.venta.fechaVen = compra.fecha_vencimiento;
                                vue.venta.serie = compra.serie;
                                vue.venta.numero = compra.numero;
                                vue.venta.total = compra.total;
                                ...
                                data.repuestos.forEach(rep => {
                                    vue.productos.push({
                                        tipo: 'repuesto',
                                        productoid: rep.id_repuesto,
                                        nom_prod: rep.nombre,
                                        codigo: rep.codigo,
                                        codigo_app: rep.codigo,
                                ...
                                if (data.pagos && data.pagos.length > 0) {
                                    vue.venta.dias_lista = data.pagos.map(p => ({
                                        fecha: p.fecha,
                                        monto: p.monto
                                    }));
                                ...
                                $('#btnGuardar').text('Actualizar Compra');
                            } else {
                                Swal.fire('Error', data.msg || 'No se pudo cargar la compra', 'error');
                            }
                        },
                        error: function() {
                            $("#loader-menor").hide();
                ...
                guardarCompra() {
                    if (this.productos.length > 0) {
                        ...
                        if (continuar) {
                            // Agregar ID de compra para actualización
                            const compraIdEdit = document.getElementById('compra_id_edit');

                            // Mapear los campos del frontend a los nombres que espera el backend
                            const data = {
                                id_compra: compraIdEdit ? compraIdEdit.value : null,
                                id_proveedor: this.venta.id_proveedor,
                                fecha_emision: this.venta.fecha,
                                fecha_vencimiento: this.venta.fechaVen,
                            ...
                            _ajax("/ajs/compra/actualizar", "POST",
                                data,
                                function (resp) {
                                    $("#loader-menor").hide();
                                    if (resp.resp || resp.res) {
                                        Swal.fire({
                                            icon: 'success',
                                            title: 'Bien',
                                        ...
                                        alertAdvertencia('Alerta', resp.msg || 'No se pudo actualizar la compra')
                                    }
                                }
                            );
                        } else {
        ...
        $("#input_datos_cliente").autocomplete({
            ...
            select: function (event, ui) {
                ...
                app._data.venta.id_proveedor = ui.item.codigo
            }
        })
