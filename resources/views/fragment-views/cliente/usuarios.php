    ...
    <div class="col-12">
        <div class="card border-0 shadow-sm" style="border-radius:12px;">
            <div class="card-title-desc text-end" style="padding: 20px 10px 0 0;">
                <button type="button" id="manage-roles" class="btn border-rojo me-2"><i
                ...
                <form id="rolForm">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-8">
                            ...
                            <input type="text" name="nombre" id="nombre_rol" class="form-control form-control-sm"
                                required>
                        </div>
                        <div class="col-md-4">
                            <div class="d-flex flex-column h-100 justify-content-end">
                                <div class="form-check form-switch mb-2">
                                    <input class="form-check-input" type="checkbox" id="ver_precios" name="ver_precios"
                                        value="1">
                                ...
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="puede_eliminar"
                                        name="puede_eliminar" value="1">
...
console.log('Submódulos seleccionados:', submodulosSeleccionados);
...
$.ajax({
    ...
    success: function (response) {
        ...
        modulosIzquierda.forEach(function (modulo) {
            // CORRECCIÓN: Verificar si el módulo está seleccionado
            const moduloChecked = modulosSeleccionados.includes(modulo.id) ? 'checked' : '';
            console.log(`Módulo ${modulo.id} (${modulo.nombre}): ${moduloChecked ? 'MARCADO' : 'NO MARCADO'}`);
                ...
                <div class="modulo-header">
                    <div class="form-check">
                        <input class="form-check-input modulo-check" type="checkbox" 
                            ...
                            id="modulo_${modulo.id}" ${moduloChecked}
            ...
            if (tieneSubmodulos) {
                htmlIzquierda += `<div class="submodulos-container" id="submodulos_${modulo.id}"${moduloChecked ? '' : ' style="display:none;"'}>`;
                modulo.submodulos.forEach(function (submodulo) {
                    const submoduloChecked = submodulosSeleccionados.includes(`${modulo.id}|${submodulo.id}`) ? 'checked' : '';
                    console.log(`Submódulo ${modulo.id}|${submodulo.id}: ${submoduloChecked ? 'MARCADO' : 'NO MARCADO'}`);
                    
                    htmlIzquierda += `
                    <div class="submodulo-item">
                        <div class="form-check">
                            <input class="form-check-input submodulo-check" type="checkbox" 
                                name="submodulos[]" value="${modulo.id}|${submodulo.id}" 
                                id="submodulo_${modulo.id}_${submodulo.id}" ${submoduloChecked}
                                data-modulo="${modulo.id}">
                            <label class="form-check-label" for="submodulo_${modulo.id}_${submodulo.id}">
        ...
        modulosDerecha.forEach(function (modulo) {
            ...
            if (tieneSubmodulos) {
                htmlDerecha += `<div class="submodulos-container" id="submodulos_${modulo.id}"${moduloChecked ? '' : ' style="display:none;"'}>`;
                modulo.submodulos.forEach(function (submodulo) {
                    const submoduloChecked = submodulosSeleccionados.includes(`${modulo.id}|${submodulo.id}`) ? 'checked' : '';
                    console.log(`Submódulo ${modulo.id}|${submodulo.id}: ${submoduloChecked ? 'MARCADO' : 'NO MARCADO'}`);
